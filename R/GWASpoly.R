...
GWASpoly <- function(data,models,traits=NULL,params=NULL,n.core=1,quiet=F) {
  ...
  for (j in 1:n.trait) {
    ...
    if (params$P3D) {
      ...
      if (n.core > 1) {
        tmp <- parLapply(cl=cl,X=K,fun=function(K,other){
          mixed.solve(y=other$y,
                      X=other$X,
                      Z=other$Z,
                      K=K,
    ...
    for (k in 1:n.model) {
      ...
      if (params$P3D) {  
        if (n.core > 1) {
          score.list <- parLapply(cl,X=data2,fun=function(x,other){
            .score.calc(data@geno[,x$markers,drop=FALSE],
                        y=other$y,
                        Z=other$Z,
                        X=other$X,
                        K=NULL,
